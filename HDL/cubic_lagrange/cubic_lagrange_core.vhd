...
end entity cubic_lagrange_core;

architecture rtl of cubic_lagrange_core is

    -- Constants
    constant ROUND_CONST_16 : signed(40 downto 0) := to_signed(32768, 41);
    constant RECIP_6_Q16    : signed(16 downto 0) := to_signed(10923, 17); -- round(65536 / 6)

    -- Stage 1: Coefficient Generation (scaled by 6)
    ...
    -- Stage 8: Multiply 4 (prod_div = T3 * 10923) -> Maps to DSP48 MREG
    signal val_st8 : std_logic := '0';
    signal prod_div_i_st8, prod_div_q_st8 : signed(40 downto 0) := (others => '0');

    -- Stage 9: Scale and Saturate (res = (prod_div + 32768) >> 16) -> Maps to DSP48 PREG
    signal val_st9 : std_logic := '0';
    signal res_i_st9, res_q_st9 : signed(15 downto 0) := (others => '0');
    ...
    begin
        if rising_edge(clk) then
            if rst_n = '0' then
                ...
                -----------------------------------------------------------------
                -- STAGE 1: Exact Integer Coefficients (scaled by 6, shift-add)
                -----------------------------------------------------------------
                val_st1 <= in_valid;
                mu_st1  <= signed(resize(mu, 17));

                -- I Channel
                c0_i_st1 <= shift_left(resize(y0_i, 24), 2) + shift_left(resize(y0_i, 24), 1);
                c1_i_st1 <= -shift_left(resize(ym1_i, 24), 1) - (shift_left(resize(y0_i, 24), 1) + resize(y0_i, 24)) + (shift_left(resize(y1_i, 24), 2) + shift_left(resize(y1_i, 24), 1)) - resize(y2_i, 24);
                c2_i_st1 <= (shift_left(resize(ym1_i, 24), 1) + resize(ym1_i, 24)) - (shift_left(resize(y0_i, 24), 2) + shift_left(resize(y0_i, 24), 1)) + (shift_left(resize(y1_i, 24), 1) + resize(y1_i, 24));
                ...
                -- Q Channel
                c0_q_st1 <= shift_left(resize(y0_q, 24), 2) + shift_left(resize(y0_q, 24), 1);
                c1_q_st1 <= -shift_left(resize(ym1_q, 24), 1) - (shift_left(resize(y0_q, 24), 1) + resize(y0_q, 24)) + (shift_left(resize(y1_q, 24), 2) + shift_left(resize(y1_q, 24), 1)) - resize(y2_q, 24);
                c2_q_st1 <= (shift_left(resize(ym1_q, 24), 1) + resize(ym1_q, 24)) - (shift_left(resize(y0_q, 24), 2) + shift_left(resize(y0_q, 24), 1)) + (shift_left(resize(y1_q, 24), 1) + resize(y1_q, 24));
                ...
                -----------------------------------------------------------------
                -- STAGE 8: Multiplier 4 for Division by 6 (prod_div = T3 * 10923)
                -----------------------------------------------------------------
                val_st8        <= val_st7;
                prod_div_i_st8 <= t3_i_st7 * RECIP_6_Q16;
                prod_div_q_st8 <= t3_q_st7 * RECIP_6_Q16;

                -----------------------------------------------------------------
                -- STAGE 9: Scale by 2^16 and Saturate to Q1.15
                -----------------------------------------------------------------
                val_st9   <= val_st8;
                res_i_st9 <= saturate_q15(resize(shift_right(prod_div_i_st8 + ROUND_CONST_16, 16), 24));
                res_q_st9 <= saturate_q15(resize(shift_right(prod_div_q_st8 + ROUND_CONST_16, 16), 24));
