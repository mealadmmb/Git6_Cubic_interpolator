function y_out = cubic_catmull_interp(ym1, y0, y1, y2, mu_u16)
% CUBIC_CATMULL_INTERP Bit-true fixed-point (Q1.15) Catmull-Rom cubic interpolator.
%
% Inputs:
...
%   y_out           : int16 interpolated sample in Q1.15 format
    ...
    % Convert inputs to double for 64-bit integer simulation
    ym1_d = double(ym1);
    y0_d  = double(y0);
    y1_d  = double(y1);
    y2_d  = double(y2);
    ...
    % Exact integer coefficients (scaled by 2):
    % 2*a0 = 2*y0
    % 2*a1 = y1 - ym1
    % 2*a2 = 2*ym1 - 5*y0 + 4*y1 - y2
    % 2*a3 = -ym1 + 3*y0 - 3*y1 + y2
    c0 = 2 * y0_d;
    c1 = y1_d - ym1_d;
    c2 = 2 * ym1_d - 5 * y0_d + 4 * y1_d - y2_d;
    ...
    % Final division by 2 with rounding: (t3 + 1) >> 1
    y_scaled = floor((t3 + 1) / 2);
